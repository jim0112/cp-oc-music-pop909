...
from oc_music_transformer import MusicTransformer
from oc_dataloader_full_song import REMIFullSongTransformerDataset
from torch.utils.data import DataLoader
from oc_validate import validate
...
def train_model(epoch, model, dloader, optim, sched):
  ...
  for batch_idx, batch_samples in enumerate(dloader):
    ...
    y_pitch, y_vel, y_dur, y_pos, y_bar = model(batch_dec_inp, batch_dec_tgt.permute(1, 0, 2))
    ...
    y_pos = y_pos[:,...].permute(1, 2, 0)
    y_bar = y_bar[:,...].permute(1, 2, 0)
    print(y_pitch.size(), y_vel.size(), y_dur.size(), y_pos.size(), y_bar.size())
    ...
    print ('--pitch loss {:.4f}, --vel loss {:.4f}, --dur loss {:.4f}, --pos loss {:.4f}, --bar loss {:.4f}'.format(loss_pitch, loss_vel, loss_dur, loss_pos, loss_bar))
...
if __name__ == "__main__":
  dset = REMIFullSongTransformerDataset(
    './octuple_dataset', 'octuple_vocab/pop909_octuple_Pitch_vocab.pkl', 
  ...
  dloader = DataLoader(dset, batch_size=2, shuffle=True, num_workers=16)
  val_dloader = DataLoader(val_dset, batch_size=2, shuffle=True, num_workers=16)
  n_token = [90, 65, 18, 18, 236] 
