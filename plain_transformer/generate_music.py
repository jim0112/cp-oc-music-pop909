    ...
    if events[i].name == 'Bar':
      ...
      temp_tempos.append(TempoEvent(
        int(events[i].value), cur_bar, cur_position
         ...
         (i+1) < len(events) and 'Note_Duration' in events[i+1].name and 'Note_Velocity' in events[i+2].name:      
      temp_notes.append(
        NoteEvent(
...
if __name__ == "__main__":
  model = torch.load("results/param/ep640_loss0.809_params.pt")
  model.eval()
  f = "pop909_vocab.pkl"
  event2idx = pickle.load(open(f, 'rb'))[0]
  idx2event = pickle.load(open(f, 'rb'))[1]
  event , _ = generate_fast(model, event2idx, idx2event)
