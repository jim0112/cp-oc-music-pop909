  ...
  with torch.no_grad():
    for r in range(rounds):
      ...
      for batch_idx, batch_samples in enumerate(dloader):
        ...
        loss = (loss_pitch + loss_vel + loss_dur + loss_pos + loss_bar) / 5
        # print ('[output shapes] mu: {}, logvar: {}, logits: {}'.format(
        #   mu.size(), logvar.size(), dec_logits.size()
        # ))
        print('[pitch loss] : {:.4f}, [velocity loss] : {:.4f}, [duration loss] : {:.4f}, [position loss] : {:.4f}, [bar loss] : {:.4f}'.format(loss_pitch, loss_vel, loss_dur, loss_pos, loss_bar))
