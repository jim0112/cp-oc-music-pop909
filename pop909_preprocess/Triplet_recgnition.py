...
root_dir = 'POP909'
melody_out_dir = 'new_pop909_melody_midi'
...
def align_notes_to_secs(midi_obj):
  ...
  #print('len of accompany notes :', len(accompany_notes))
...
def quantize_notes(notes, beat_times, downbeat_idx):
  ...
  return quantized
def triplet_recognition(target_notes):
  for notes in target_notes:
    if len(notes) == 3:
      if notes[0]['dur_tick'] <= 160 and notes[1]['dur_tick'] <= 160 and notes[2]['dur_tick'] <= 160:
        if notes[1]['st_tick'] - notes[0]['st_tick'] == 160 and notes[2]['st_tick'] - notes[1]['st_tick'] == 160:
          for note in notes:
              note['velocity'] += 127
  return target_notes
def merge_and_resolve_polyphony(vocal_notes, piano_notes, accompany_notes):
  vocal_notes = list(chain(*vocal_notes))
  piano_notes = list(chain(*piano_notes))
  accompany_notes = list(chain(*accompany_notes))
  notes = sorted(
...
def align_midi_beats(piece_dir):
  ...
  vocal_notes, piano_notes, accompany_notes = align_notes_to_secs(midi_obj)
  vocal_notes = group_notes_per_beat(vocal_notes, midi_beat_times)
  piano_notes = group_notes_per_beat(piano_notes, midi_beat_times)
  accompany_notes = group_notes_per_beat(accompany_notes, midi_beat_times)
  # print (vocal_notes[:4])
  # print (piano_notes[:4])
  ...
  accompany_notes = quantize_notes(accompany_notes, midi_beat_times, downbeat_idx)
  # print (piano_notes[:8])
  # print ('[# notes]', vocal_notes + piano_notes)
  vocal_notes = triplet_recognition(vocal_notes)
  piano_notes = triplet_recognition(piano_notes)
  final_notes, final_rec = merge_and_resolve_polyphony(vocal_notes, piano_notes, accompany_notes)
...
if __name__ == '__main__':
  ...
  for pdir in pieces_dir:
    notes_rec, tri_rec = align_midi_beats(
      os.path.join(root_dir, pdir)
    )
    midi_obj = miditoolkit.midi.MidiFile(
                  os.path.join(melody_out_dir, pdir.split('/')[-1] + '_melody.mid')
                )
...
# 4/4 operation
    flag1 = 0
    flag2 = 0
    for index, n in enumerate(notes_rec):
      bar = n['st_tick'] // (4 * DEFAULT_TICKS_PER_BEAT)
      duration = min(n['dur_tick'], 4 * DEFAULT_TICKS_PER_BEAT)
      # print (' --', bar, position, duration)
      if flag1 < len(chord_marker) and chord_marker[flag1].time <= n['st_tick']:
        ...
        flag1 += 1
      if tri_rec[index] == 1:
        position = ( n['st_tick'] % (4 * DEFAULT_TICKS_PER_BEAT) ) // 160
        notes[ bar ].append({'name': 'Beat', 'value': '12_{}'.format(position)})
      else:
        position = ( n['st_tick'] % (4 * DEFAULT_TICKS_PER_BEAT) ) // 120
...
# to ensure everything is correct(more than 1 attrs)
    ...
    pickle.dump(
       ...
       open(os.path.join(out_dir, pdir + '.pkl'), 'wb'),
