...
root_dir = 'POP909'
melody_out_dir = 'multitrack_pop909_melody_midi'
...
def align_notes_to_secs(midi_obj):
  ...
  for note in midi_obj.instruments[0].notes:
    ...
    vocal_notes.append(
      {'st_sec': note_st_sec, 'ed_sec': note_ed_sec, 'pitch': note.pitch, 'velocity': note.velocity, 'track' : 0}
    )
  ...
  for note in midi_obj.instruments[1].notes:
    ...
    # print (note_st_sec, note_ed_sec)
    piano_notes.append(
      {'st_sec': note_st_sec, 'ed_sec': note_ed_sec, 'pitch': note.pitch, 'velocity': note.velocity, 'track' : 1}
    )
  accompany_notes = []
  for note in midi_obj.instruments[2].notes:
    ...
    {'st_sec': note_st_sec, 'ed_sec': note_ed_sec, 'pitch': note.pitch, 'velocity': note.velocity, 'track' : 2}
    )
...
def merge_and_resolve_polyphony(vocal_notes, piano_notes, accompany_notes):
  ...
  return final_notes
# clear
def dump_melody_midi(note_0, note_1, note_2, bpm, midi_out_path):
  ...
  midi_obj.instruments = [miditoolkit.midi.Instrument(0, name='piano_melody'), 
                          miditoolkit.midi.Instrument(1, name='piano_melody'),
                          miditoolkit.midi.Instrument(2, name='piano_melody')]
...
def align_midi_beats(piece_dir):
  ...
  #final_notes = merge_and_resolve_polyphony(vocal_notes, piano_notes, accompany_notes)
  ...
  dump_melody_midi(
    vocal_notes, piano_notes, accompany_notes, final_bpm,
